 ...
 */
var WeatherObject = (function () {
    function WeatherObject(units, tempUnits, windUnits, useKmh) {
        this.units = units;
    ...
    WeatherObject.prototype.beaufortWindSpeed = function () {
        var windInKmh = this.windUnits === "imperial" ? this.windSpeed * 1.609344 : this.useKmh ? this.windSpeed : (this.windSpeed * 60 * 60) / 1000;
        var speeds = [1, 5, 11, 19, 28, 38, 49, 61, 74, 88, 102, 117, 1000];
        for (var _i = 0, _a = speeds.entries(); _i < _a.length; _i++) {
            var _b = _a[_i], index = _b[0], speed = _b[1];
            if (speed > windInKmh) {
                return index;
            }
        }
    ...
    WeatherObject.prototype.kmhWindSpeed = function () {
        var windInKmh = this.windUnits === "imperial" ? this.windSpeed * 1.609344 : (this.windSpeed * 60 * 60) / 1000;
        return windInKmh;
    };
    WeatherObject.prototype.nextSunAction = function () {
        return moment().isBetween(this.sunrise, this.sunset) ? "sunset" : "sunrise";
    };
    WeatherObject.prototype.feelsLike = function () {
        if (this.feelsLikeTemp) {
            return this.feelsLikeTemp;
        }
        var windInMph = this.windUnits === "imperial" ? this.windSpeed : this.windSpeed * 2.23694;
        var tempInF = this.tempUnits === "imperial" ? this.temperature : (this.temperature * 9) / 5 + 32;
