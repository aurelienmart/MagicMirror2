...
Module.register("newsfeed", {
	...
	getDom: function () {
		...
		if (this.newsItems.length > 0) {
			...
			if (this.isShowingDescription) {
				...
				description.innerHTML = " &bull; " + (this.config.truncDescription ? (txtDesc.length > this.config.lengthDescription ? txtDesc.substring(0, this.config.lengthDescription) + "..." : txtDesc) : txtDesc);
	...
	scheduleUpdateInterval: function () {
		...
		if (this.config.broadcastNewsFeeds) {
			this.sendNotification("NEWS_FEED", { items: self.newsItems });
		}
