...
var CalendarFetcher = function (url, reloadInterval, excludedEvents, maximumEntries, maximumNumberOfDays, auth, includePastEvents) {
				...
				if (event.type === "VEVENT") {
					...
					if (typeof event.rrule !== "undefined" && event.rrule !== null && !isFacebookBirthday) {
						...
						for (var d in dates) {
							...
							var adjustDays = getCorrection(event, date);
						...
						var adjustDays = getCorrection(event, startDate.toDate());
	 ...
	 *
	 */
	var getCorrection = function (event, date) {
		var adjustHours = 0;
		...
		if (event.start.tz) {
			...
			var mmo = 0; // offset  from TZ string or calculated
			var mm = 0; // date with tz or offset
			var mms = 0; // utc offset of created with tz
